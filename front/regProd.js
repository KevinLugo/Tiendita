...
document.getElementById("boton1").addEventListener("click", async () => {
  ...
  if(codBar == "" ||  nomProd == "" || fechaCad == "" || idCat == "" || idProv == "" || pCompra == "" || pVenta == ""
    || sActual == "" || sMin == "") {
    ...
    if (errorCB == true) {
      ...
      let DP={ codBar,nomProd,idCat,idProv,pCompra,pVenta,sActual,sMin,idUsuario,fechaCad}
      //aca ingreso los datos del producto
      try {
        const res = await fetch("http://localhost:3000/api/regProd", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
...
//aqui pongo que la fecha maxima sea a partir de oi
const hoy = new Date();
